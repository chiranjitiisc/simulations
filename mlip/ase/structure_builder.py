...
## system
box = np.array([215,430,430])
region1 = np.array([[0,215],[0,430],[0,430]])                                 # lo, hi for all 3 aixs for mol1
region2 = np.array([[0,215],[0,430],[0,430]])                                 # for mol2
...
s2 = read("s2_single.xyz")
mo3o9 = read("mo3o9_single.xyz")
...
for i in range(n_mo3o9 + n_s2):

    # Randomly rotate molecule
    trial1 = rotation(mo3o9, rn)
    trial2 = rotation(s2,rn)
...
write("data.lmp", sys, 
      format="lammps-data", 
      specorder=["Mo","O","S"],    # list of chemical species present 
      units="metal", 
      atom_style="charge", 
      masses=True,
      )
